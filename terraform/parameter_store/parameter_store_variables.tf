variable "twitter_consumer_key" {
  type      = string
  default = "FU3vosZUkBR9RH5EPBd2P6mT4"
}

variable "twitter_consumer_key_secret" {
  type      = string
  default = "wrpQh8HaEDfK0c4XgnJyl91gVsVUD0fNcIVAKilgyqarstdHNX"
}

variable "twitter_access_token" {
  type      = string
  default = "1494383698220888067-mYeSUeNeGhuJhm4aK0GJu0boAwmdlM"
}

variable "twitter_access_token_secret" {
  type      = string
  default = "A5yrS2j0iUyI2gNhfzM2LdDGySSlxXW53oEiwp5C35Q4Z"
}

variable "linkedin_access_token" {
  type      = string
  default = "AQX7oQ5vhByPzUVJL_iRZam3npq4wonMnbl06txISKpfo7hKPsLc-6CXhUCeTG_6cvskt2ofBgS8RSZxSqOYZYXAprfBBbUNlVCSMcST2CWBRpCbRp7NrOMfBjTQZrnSzNg7AQPJUhPqjzmzBbogE7FuhmdwhWBTPp2TtF4cyM1Pc7sIRKHRQGCk8BLaPzburiVkB4SYeKQoIBvbncX9TWMZ3f5j0bwmXL5Ci_fbsw7vhc3ZWxLUNJjwcSX3iVsjtCKjADxtMzm16M97oEQ6e0gY4iU92LRud_c64Eivccnhk1mxwk6fd2eSJ7o-W8T2rnJYu_QavTxCiLbMQpyvtgv36nbI-w"
}

variable "facebook_page_access_token" {
  type      = string
  default = "EAAFZBmZB3oSg8BAOoznX3TKnSQvvYfJogp7Dr2hrlZCZBZBNg61sIVGhqSxhLXgAdHhCDMMhumS4n7YhZAbqssphQNT0nqeffdhoDZBx41bKmmc8BZBjVC1W6AuHCbZAhlA1m7tC4M5i7crfda3RHJ2MZAhDT1vx84auYRuMDl76mGA0hO7cptZACEzLMQuj35YKra6HLUauOjlnXpPBq5Je4YHU42gfuR2XTQZD"
}

variable "facebook_user_access_token" {
  type      = string
  default = ""
}

variable "notion_access_token" {
  type      = string
  default = "secret_dtG5JWfd9ifrfV4FMqneH0AGo63AS943ovHfZu73IL7"
}
